...
// Send one or more packets. Takes either a message definition and optional
// parameters, or a Buffer. Can also take a function in order to gather
// multiple packets in a single write.
proto.write = function(block) {
  // Single message invocation (no block).
  if (typeof(block) !== 'function') {
    var args = arguments;
    return this.write(function(writer) {
  ...
  // The writer function passed to the block.
  var writer = function(message, params, callback) {
    var payload;

    // Allow passing a buffer as payload straight up. This, of course, requires
    // that the caller properly set the first byte as the ID.
...
var TransportServer = function(options, listener) {
  ...
  net.Server.call(this, function(conn) {
    ...
    tspt.on('request', function(name, writer) {
      self.emit('request', tspt, name, writer);
    });
