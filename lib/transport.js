...
proto._packetLoop = function() {
  ...
  this._ondata = function() {
    ...
    // Verify the MAC.
    if (self._rx.macSize) {
      var theirMac = buffers.splice(0, self._rx.macSize).toBuffer();
    ...
    // FIXME: disallow non-kex messages during kex.
...
proto.write = function(block) {
  ...
  block(function(type, params) {
    ...
    // FIXME: queue non-kex messages during kex.
