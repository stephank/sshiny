...
proto._packetLoop = function() {
  ...
  this._ondata = function() {
    ...
    if (waitFor === null) {
      ...
      waitFor = 4 + data.readUInt32BE(0) + self._rx.mac.size;
      // FIXME: verify input
    }
    ...
    if (self._rx.mac.size) {
      var mac = packet.slice(packet.length - self._rx.mac.size);
      // FIXME: verify MAC
    }
    ...
    if (message) {
      ...
      var payload = packet.slice(5, packet.length - self._rx.mac.size - padding);
