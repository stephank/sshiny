...
messages.register({
  ...
  handle: function(tspt, params, payload) {
    var theirRole = (tspt.role === 'client' ? 'server' : 'client');

    if (tspt._kex && tspt._kex[theirRole + 'Params']) {
      // FIXME: bail
      return;
    }
    ...
    tspt.write(function(writer) {
      ...
      tspt._kex[theirRole + 'Params']  = params;
      tspt._kex[theirRole + 'Payload'] = payload;
